// Mylearning.jsx - COMPLETELY FIXED VERSION
...
const MyLearning = () => {
  ...
  const fetchLearningProgress = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Don't throw error, just set empty progress
        setProgressData({});
      }
    } catch (error) {
      console.error('Error fetching progress:', error);
      // Don't let progress errors break the UI
      setProgressData({});
    }
  };
  ...
  const handleDocumentView = async (document) => {
    try {
      ...
      const isOffice = ['doc', 'docx', 'ppt', 'pptx', 'xls', 'xlsx'].includes(document.file_type?.toLowerCase());
      ...
      const shouldDownload = window.confirm(`Unable to view document: ${error.message}\n\nWould you like to download it instead?`);
      if (shouldDownload) {
        await handleDocumentDownload(document);
      }
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          if (errorData.error === 'File not found on server') {
            errorMessage = `The document "${document.title}" is missing from the server. Please contact the teacher to re-upload it.`;
          }
        } catch (e) {
      ...
      if (!filename) {
        filename = document.original_filename || `${document.title || 'document'}.${document.file_type || 'pdf'}`;
      }

      const blob = await response.blob();
      const blobUrl = window.URL.createObjectURL(blob);
      
      // Create download link - FIXED: Ensure we're in browser environment
      ...
      if (selectedCategory) {
        await markAsCompleted(selectedCategory.course_category, 'documents', docId);
      }
      
    } catch (error) {
      console.error('❌ Download error:', error);
      alert(error.message || 'Download failed. Please try again or contact support.');
    } finally {
  ...
  const markAsCompleted = async (category, materialType, materialId) => {
    try {
      ...
      if (response.ok) {
        ...
        console.warn('Failed to mark as completed:', response.status);
...
export default MyLearning;// Mylearning.jsx - COMPLETELY FIXED VERSION
